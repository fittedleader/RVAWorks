...
import styles from "./businessPage.module.css"; // Importing styles
  ...
  return (
    <nav className="navbar">
      ...
      <ul className={styles["nav-links"]}>
          <li><a className={styles["nav-btn-green"]} href="#mission">Mission</a></li>
          <li><a className={styles["nav-btn-orange"]} href="#resources">Resources</a></li>
          <li><a className={styles["nav-btn-green"]} href="#impact">Impact</a></li>
          <li><a className={styles["nav-btn-orange"]} href="#programs">Programs</a></li>
      </ul>
      <div className={styles["cta-buttons"]}>
          <button className={styles["subscribe-btn"]} onClick={() => toggleModal('subscribe')}>Subscribe</button>
          <button className={styles["donate-btn"]} onClick={() => window.open('https://www.paypal.com/paypalme/rvaworks?locale.x=en_US', '_blank')}>Donate</button>
      </div>
      ...
      {isDropdownOpen && (
        <div className="dropdown-menu">
          <ul>
            ...
            <li><a className="nav-btn-orange" href="#resources">Resources</a></li>
            <li><a className="nav-btn-green" href="#impact">Impact</a></li>
            <li><a className="nav-btn-orange" href="#programs">Programs</a></li>
            <li><button className="subscribe-btn" onClick={() => toggleModal('subscribe')}>Subscribe</button></li>
...
function HeroSection() {
    ...
    return (
        <header className={styles.hero}>
            ...
            <p className={styles["hero-subtitle"]}>
                <i>RVA Works Enterprise Support, Inc is a tax exempt organization</i>
            </p>
...
function MissionSection() {
    return (
        <section id="mission" className={styles.mission}>
            ...
            <h2>Our Mission</h2>
            <h3>
                <span className={styles["text-green"]}>RVA</span> <span className={styles["text-orange"]}>Works</span> is a nonprofit in Richmond, Virginia that helps entrepreneurs start businesses. <span className={styles["text-green"]}>RVA</span> <span className={styles["text-orange"]}>Works</span> offers a variety of resources such as business training and network building to help entrepreneurs at all levels to grow their businesses. <span className={styles["text-green"]}>RVA</span> <span className={styles["text-orange"]}>Works'</span> mission is to eliminate barriers to business ownership to give everyone a chance to become a successful entrepreneur. We provide educational and financial services to help those in need use their passions to build a robust business. We support prospective entrepreneurs of all backgrounds. We make it our goal to allow everyone the opportunity to learn more about business to help them grow their passions. Regardless of one's business stage, industry, or revenue, <span className={styles["text-green"]}>RVA</span> <span className={styles["text-orange"]}>Works</span> offers resources for all.
            </h3>
        </section>
    );
}
...
function DirectorySection() {
  ...
  useEffect(() => {
      fetch('/rvaworksdatatablegood.xlsx') 
          .then(response => {
              if (!response.ok) {
          ...
          .then(buffer => {
              ...
              const companiesData = data.slice(1).map(row => {
  ...
  const findMatchingCompanies = () => {
    let selectedResources = Array.from(document.querySelectorAll('#resources input:checked')).map(checkbox => checkbox.value);
    console.log("Selected Resources Array:", selectedResources);
    if (selectedResources.length === 0) {
      selectedResources = ['Workspace', 'Networking, Expos & Referrals', 'Accelerator', 'High School & Youth Programs', 'Certificates, Workshops & Training', 'Mentoring & Consulting', 'Direct Capital'];
    }

    const selectedIndustry = document.getElementById('industry').value.trim();
    console.log("Selected Industry:", selectedIndustry);

    // Filter companies based on the selected resources
    const matchingCompanies = companies.filter(company => 
      selectedResources.some(resource => company[resource] === 1)
    );
    console.log("Before results", matchingCompanies)

    const industryFilteredCompanies = matchingCompanies.filter(company => {
      const affiliations = company["Industry Affiliations"] || "";

      // Special cases for All and exclusions
      if (affiliations.includes("All ventures, excluding real estate") && selectedIndustry !== "Real Estate") {
        return true; // Matches any industry except Real Estate
      }
      if (affiliations.includes("All ventures, excluding real estate") && selectedIndustry === "Real Estate") {
        return false;
      }
      if (affiliations.includes("All")) {
          return true; // Matches any industry
      }
      

      // Handle multiple affiliations separated by commas
      const affiliationArray = affiliations.split(",").map(a => a.trim());

      // Check if the affiliation requires multiple tags (e.g., "Technology||Enterprise Technology")
      const multiTagMatches = affiliationArray.some(affiliation => {
          if (affiliation.includes("||")) {
              const requiredTags = affiliation.split("||").map(tag => tag.trim());
              return requiredTags.every(tag => tag === selectedIndustry);
          }
          return false;
      });

      // Exact matches or multi-tag matches
      return affiliationArray.includes(selectedIndustry) || multiTagMatches;
    });

    // Map to only include necessary fields
    const filteredResults = industryFilteredCompanies.map(company => ({
      organization: company["Organization Name"],
      website: company["Website"],
      telephone: company["Telephone"],
  }));

    console.log("Matching Companies:", filteredResults);
    setResults(filteredResults); // Set state for further use or display
  ...
  const HandleConnect = () => {
      ...
      const resources = Array.from(document.querySelectorAll('#resources input:checked'))
      ...
      fetch('https://rva-works-server.vercel.app/write-to-sheet', {
          ...
          .then(data => {
              console.log('Data successfully written to the sheet:', data);
              // Reset the form after successful submission
              resetForm();
          })
          .catch(error => console.error(error));
        
      const matchingCompanies = findMatchingCompanies(resources, companies);
      console.log("ACTUAL COMPANIES: ", matchingCompanies)
      resetForm();
      // You can now use the matchingCompanies data as needed
      setResults(matchingCompanies);
      if (setFormError == false || setFormError !== true) {
        navigate("/filtered-companies", { state: { results: matchingCompanies } });
      }
  };

  const resetForm = () => {
      document.getElementById('industry').value = 'None';
      document.querySelectorAll('#resources input').forEach(input => (input.checked = false));
  };

  return (
    <section id="resources" className="resources-section">
      <div class="divider"></div>
      <h2 className={styles["resources-section-h2"]}>Directory of Richmond Resources</h2>
      ...
      <div className={styles["centered-container"]}>
          ...
          <div className={styles["resources-section"]}>
              ...
              <label className={styles["label"]}>Industry:
                <select id="industry" className={styles["select"]}>
                  <option value="None">Choose an Industry</option>
                  ...
                  <option value="Consumer Goods and Retail||Food & Beverage">Consumer Goods and Retail</option>
                  ...
                  <option value="Healthcare">Healthcare and Medical Services</option>
                  <option value="Hospitality and Tourism">Hospitality and Tourism</option>
                  <option value="Technology||Enterprise Technology">Information Technology</option>
                  ...
                  <option value="Professional Servicess">Professional Services (e.g., legal, accounting, consulting)</option>
                </select>
              </label>

              <fieldset id="resources" className={styles["fieldset"]}>
                <h3>Find Resources:</h3>
                <label>
                  <input type="checkbox" value="Networking, Expos & Referrals" />Networking Events
                </label>
                <label>
                  <input type="checkbox" value="High School & Youth Programs" /> Classroom Instruction
                </label>
                <label>
                  <input type="checkbox" value="Direct Capital" /> Startup Finance
                </label>
                <label>
                  <input type="checkbox" value="Workspace" /> Physical Spaces & Equipment
                </label>
                <label>
                  <input type="checkbox" value="Accelerator" /> Market Research & Data
                </label>
                <label>
                  <input type="checkbox" value="Mentoring & Consulting" /> Mentorship
                </label>
                <label>
                  <input type="checkbox" value="Certificates, Workshops & Training" /> Other Support Services
                </label>
              </fieldset>

              <button onClick={HandleConnect} className={styles["connect-button"]}>
                    Connect
              </button>
              {formError && (
...
function ResourcesSection() {
  ...
  const HandleConnect = () => {
      ...
      const operatingBusiness = document.getElementById('operating-business').value.trim();
      const businessPlan = document.getElementById('business-plan').value.trim();
      const coachMentor = document.getElementById('coach-mentor').value.trim();
      const hiring = document.getElementById('hiring').value.trim();
      const planToSell = document.getElementById('plan-to-sell').value.trim();
      const experience = document.getElementById('experience').value.trim();
      const incomeLevel = document.getElementById('income-level').value.trim();
      const businessGoal = document.getElementById('business-goal').value.trim();
      // Validate required fields
      const timestamp = new Date().toLocaleString();
      ...
      // Validate required fields
      if (!name || !email || !phone || !address || !incomeLevel ) {
  ...
  const resetForm = () => {
      ...
      document.getElementById('operating-business').value = 'Y';
      document.getElementById('business-plan').value = 'Y';
      document.getElementById('coach-mentor').value = 'Y';
      document.getElementById('hiring').value = 'Y';
      document.getElementById('plan-to-sell').value = '';
      document.getElementById('experience').value = '';
      document.getElementById('income-level').value = '$20,000 or less';
      document.getElementById('business-goal').value = '';
  };

  return (
    <section id="resources" className="resources-section">
      <div class="divider"></div>
      <h2 className={styles["resources-section-h2"]}>Apply Now</h2>
      ...
      <div className={styles["centered-container"]}>
          ...
          <div className={styles["resources-section"]}>
              ...
              <label className={styles["label"]}>
                Name: 
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <input type="text" id="name" required className={styles["input"]} />
              </label>
              

              <label className={styles["label"]}>Business Name (optional):
                  <input type="text" id="business-name" className={styles["input"]} />
              </label>
              ...
              <label className={styles["label"]}>
                Phone Number: 
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <input type="text" id="phone" required className={styles["input"]} />
              </label>
              ...
              <label className={styles["label"]}>
                ...
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <select id="operating-business" className={styles["select"]}>
              ...
              <label className={styles["label"]}>
                ...
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <select id="business-plan" className={styles["select"]}>
                  <option value="Y">Yes</option>
                  <option value="N">No</option>
                </select>
              </label>
              ...
              <label className={styles["label"]}>
                ...
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <select id="coach-mentor" className={styles["select"]}>
                  <option value="Y">Yes</option>
                  <option value="N">No</option>
                </select>
              </label>
              ...
              <label className={styles["label"]}>
                Do you anticipate hiring employees in the next three years?
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <select id="hiring" className={styles["select"]}>
                  <option value="Y">Yes</option>
                  <option value="N">No</option>
                </select>
              </label>

              <label className={styles["label"]}>What do you plan to sell, to whom, and how does it compare to competing products or services on the market?
                  <textarea id="plan-to-sell" className={styles["textarea"]}></textarea>
              </label>

              <label className={styles["label"]}>What, if any, professional experience do you have in this industry?
                  <textarea id="experience" className={styles["textarea"]}></textarea>
              </label>

              <label className={styles["label"]}>
                What is your level of household income (AGI)? 
                <p style={{ color: 'red', display: 'inline', margin: '0', paddingLeft: '5px' }}>*</p>
                <select id="income-level" required className={styles["select"]}>
                  <option value="$20,000 or less">$20,000 or less</option>
                  ...
                  <option value="$110,000 or more">$110,000 or more</option>
                </select>
              </label>

              <label className={styles["label"]}>What is your business goal for the next 3-5 years?
                  <textarea id="business-goal" className={styles["textarea"]}></textarea>
              </label>
...
const handleSubscribe = async () => {
  ...
  return (
    <footer className="footer">
      ...
      <div className="footer-info">
        ...
        <h2>Explore</h2>
        <h3>
          If you want to explore more about our business and team, visit our sponsor page.
        </h3>
      ...
      {showModal.terms && (
        ...
        >
          <div className="popup-content" onClick={(e) => e.stopPropagation()}>
            ...
            <h4>Contact Us</h4>
            <p>If you have any questions about these Temrs and Conditions, You can contact us:
              <br /><strong>RVA Works Enterprise Support, Inc.</strong>
              <br />102 UR Drive
              <br />Richmond, VA 23173 USA
              <br />(804)335-7198
              <br />donate@rvaworks.org 
              <br /><strong>Business Hours: 9am - 6pm EST</strong>
            </p>
          </div>
        </div>
      )}
  ...
  function ProgramsSection() {
    ...
    const nextSlide = () => {
      if (currentSlide < programData.length - 4) {
        setCurrentSlide(currentSlide + 1);
        sliderRef.current.style.transform = `translateX(-${(currentSlide + 1) * 21}%)`;
      } else {
        setCurrentSlide(0); 
      }
      if (currentSlide === programData.length - 5) {
        setCurrentSlide(-1);
      }
    };
